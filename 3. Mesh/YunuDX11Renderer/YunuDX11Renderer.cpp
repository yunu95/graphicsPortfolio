...
YunuDX11Renderer::YunuDX11Renderer()
    ...
    m_MeshObject(nullptr), m_TestBox(nullptr), m_TestCrate(nullptr), m_SiegeTank(nullptr)
{
...
bool YunuDX11Renderer::Initialize(HWND hWnd)
{
    ...
    m_MeshObject->LoadGeomerty(m_pASEParser->GetMesh(0));
...
void YunuDX11Renderer::Update(float deltaTime)
{
    ...
    ControlLights();
    Effects::BasicTexFX->SetDirLights(m_DirLights);
    Effects::BasicTexFX->SetPointLights(m_PointLights);
    Effects::BasicTexFX->SetSpotLights(m_SpotLights);
...
void YunuDX11Renderer::Render()
{
    ...
    */
    ...
    m_pFont->DrawTextColor(200, 100, _yellow, (TCHAR*)L"한글과 버퍼가 잘 작동하는가? %d / %f / %s", 79, 3.14f, L"노란색글씨 똠방각하 펲시 콜라 뾸뾸");
...
void YunuDX11Renderer::ControlLights()
{
