...
YunuDX11Renderer::YunuDX11Renderer()
    ...
    m_MeshObject(nullptr),m_MeshObject2(nullptr), m_TestBox(nullptr), m_TestCrate(nullptr)
{
...
bool YunuDX11Renderer::Initialize(HWND hWnd)
{
    ...
    m_MeshObject->SetRootTM(
        ...
        XMMatrixTranslation(0,0,-5)
    );
    m_MeshObject2 = new MeshObject(md3dDevice, md3dImmediateContext, mSolidRS);
    m_MeshObject2->Initialize();
    m_MeshObject2->LoadGeometry(m_pASEParser->GetMeshes());
    m_MeshObject2->LoadAnimation(m_pASEParser->m_list_animation, m_pASEParser->m_scenedata);
    m_MeshObject2->SetRootTM(
        XMMatrixScaling(0.1f, 0.1f, 0.1f) *
        XMMatrixRotationAxis({ 1,0,0 }, -1.57f)*
        XMMatrixRotationAxis({ 0,1,0 }, 3.14f)*
        XMMatrixTranslation(0,0,5)
    );
    m_MeshObject2->SetAnimElapsed(3);
