...
bool YunuDX11Renderer::Initialize(HWND hWnd)
{
    ...
    m_MeshObject = new MeshObject(md3dDevice, md3dImmediateContext, mSolidRS);
    m_MeshObject->Initialize();
    //m_MeshObject->BuildGeometryBuffers2();
    m_MeshObject->LoadGeometry(m_pASEParser->GetMeshes());
    //m_MeshObject->SetBoneLocalTM("Biped-help_R_muneca", m_MeshObject->GetBoneLocalTM("Biped-help_R_muneca") * XMMatrixScaling(0.1f, 0.1f, 0.1f));
    m_MeshObject->SetRootTM(
        XMMatrixScaling(0.1f, 0.1f, 0.1f) * XMMatrixRotationAxis({ 1,0,0 }, -1.57f)
    );
    ...
    // Siege Tank 예시
    //m_SiegeTank = new SiegeTank(md3dDevice, md3dImmediateContext, mSolidRS);
    //m_SiegeTank->Initialize();
    ...
    mDirLights[0].Diffuse = XMFLOAT4(1.0f, 0.0f, 0.f, 1.0f);
    mDirLights[0].Specular = XMFLOAT4(0.4f, 0.0f, 0.0f, 1.0f);
    ...
    mSpotLights[0].Att = XMFLOAT3(1, 0, 0.0);
...
void YunuDX11Renderer::Update(float deltaTime)
{
    ...
    m_MeshObject->SetBoneLocalTM("Biped-Head",
        XMMatrixTranslationFromVector(headTrans) *
        headRotationMatrix *
        XMMatrixScalingFromVector(headScale)
    );

    XMMatrixDecompose(
        &headScale, &headQuat, &headTrans,
        m_MeshObject->GetBoneWorldTM("Biped-Head")
    );
    auto debugAxisTM = XMMatrixScaling(5, 5, 5)* m_MeshObject->GetBoneWorldTM("Biped-Head");
    m_debugAxis->Update(debugAxisTM , m_pCamera->View(), m_pCamera->Proj());
    //m_debugAxis->Update(XMMatrixTranslationFromVector(headTrans), m_pCamera->View(), m_pCamera->Proj());
    m_MeshObject->Update(m_pCamera);
...
void YunuDX11Renderer::Render()
{
    ...
    // 테크닉 안의 각 패스별 상태적용-그리기 수행
    for (UINT p = 0; p < techDesc.Passes; ++p)
    {
        // 상수버퍼 적용(갱신), 쉐이더 바인딩, 렌더스테이트 적용(갱신) 등
...
void YunuDX11Renderer::Update_LightsControl(float delta)
{
    ...
    if (WasKeyJustPressed<'2'>())
    {
        ...
        Effects::BasicTexFX->SetPointLights(m_isPointLightActive ? mPointLights : emptyPointLights);
    }
    if (WasKeyJustPressed<'3'>())
    {
        m_isSpotLightActive = !m_isSpotLightActive;
        Effects::BasicTexFX->SetSpotLights(m_isSpotLightActive ? mSpotLights : emptySpotLights);
    }
